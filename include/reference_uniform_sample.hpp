...
namespace rct {
    ...
    public:
        ...
        reference_uniform_sample(const std::vector<value_type> &container,
            ...
            const auto ref_length = reference_size / sizeof(value_type);
            const auto block_length = block_size / sizeof(value_type);
            ...
            for (size_type i = 0; i < input_size; i += step_sample) {
                for (size_type j = 0; j < block_length; ++j) {
                    ...
                    if (!voc_reference.contains(value)) voc_reference[value] = 1;
            ...
            for (size_type i = 0; i < input_size; ++i) {
                ...
                if (value == 0) std::cout << "Error" << std::endl;
                if (!voc_reference.contains(value)) {
        ...
        //! Move constructor
        reference_uniform_sample(reference_uniform_sample &&o) noexcept {
            *this = std::move(o);
        }

        void swap(reference_uniform_sample &r) noexcept {
            if (this != &r) {
        ...
        reference_uniform_sample &operator=(reference_uniform_sample &&v) noexcept {
            if (this != &v) {
